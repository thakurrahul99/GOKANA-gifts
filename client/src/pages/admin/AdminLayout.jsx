      ...
      <aside data-lenis-prevent className={clsx('flex flex-col bg-bg-alt border-r border-[rgba(197,160,89,0.2)] transition-all duration-300 flex-shrink-0 shadow-xl overflow-y-auto', sidebarOpen ? 'w-60' : 'w-16')}>
        ...
        <nav className="flex-1 py-4 space-y-1">
          {navItems.map(({ icon: Icon, label, href }) => {
            ...
            return (
              <Link key={href} to={href} className={clsx('flex items-center gap-3 px-4 py-3 mx-2 rounded-sm transition-all duration-200 group min-h-[44px]', active ? 'bg-accent/15 text-accent border border-accent/30 font-medium' : 'text-muted hover:text-ivory hover:bg-white/5')}>
                <Icon size={18} strokeWidth={1.6} className={active ? 'text-accent' : 'text-muted group-hover:text-accent'} />
          ...
          {sidebarOpen && (
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <div className="truncate">
                ...
                <p className="text-[10px] text-muted truncate">{user?.email || 'admin@gokana.in'}</p>
              </div>
            </div>
          )}
