  ...
  return (
    <main id="main-content" className="pt-24 md:pt-28 bg-bg text-ivory min-h-screen pb-20">
      <div className="container-gokana py-8">
        ...
        <div className="mt-20 pt-16 border-t border-border"><div className="text-center mb-10"><p className="label-text text-accent mb-2">✦ More to Cherish</p><h2 className="heading-md text-ivory">You May Also Love</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{related.map((p, i) => <ProductCard key={p.id} product={p} index={i} />)}</div></div>
      </div>
      <aside className="fixed bottom-0 left-0 right-0 z-40 bg-bg/95 backdrop-blur-md border-t border-border p-3 shadow-2xl flex items-center justify-between gap-3 md:hidden" aria-label="Quick mobile checkout"><div className="pl-2"><p className="font-serif text-lg font-semibold text-accent leading-none">{formatPrice(product.price)}</p><p className="font-sans text-[11px] text-accent font-medium mt-0.5">✓ In Stock</p></div><button onClick={handleAddToCart} className="btn-primary py-2.5 px-5 text-xs flex items-center gap-1.5" aria-label="Add product to cart">{addedToCart ? <Check size={16} /> : <ShoppingBag size={16} />}{addedToCart ? 'Added!' : 'Add to Cart'}</button></aside>
    </main>
  );
}
