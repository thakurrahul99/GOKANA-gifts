      ...
      <div className="container-gokana py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-5 flex flex-col justify-start">
            ...
            {product.personalisable && <div className="mb-6 p-5 rounded-xl bg-white border border-border shadow-xs"><div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-accent mb-3"><Sparkles size={14} /><span>Complimentary Personalisation</span></div><div className="space-y-3">{personalisationFields.map((field, index) => { const key = field.label || `field-${index}`; const inputId = `personalisation-${index}`; const value = personalisation[key] || ''; const updateValue = (next) => setPersonalisation((p) => ({ ...p, [key]: next })); return <div key={inputId}><label htmlFor={inputId} className="form-label">{field.label}{field.required ? <span className="text-accent ml-1">*</span> : ''}</label>{field.type === 'textarea' ? <textarea id={inputId} rows={3} placeholder={field.placeholder || ''} value={value} onChange={(e) => updateValue(e.target.value)} className="input-field resize-none" required={field.required} /> : field.type === 'select' ? <select id={inputId} value={value} onChange={(e) => updateValue(e.target.value)} className="input-field" required={field.required}><option value="">Select an option</option>{(field.options || []).map((option) => <option key={option} value={option}>{option}</option>)}</select> : <input id={inputId} type="text" placeholder={field.placeholder || ''} value={value} onChange={(e) => updateValue(e.target.value)} className="input-field" required={field.required} />}</div>; })}</div><p className="text-[11px] text-muted mt-3">Your personalisation is saved with this cart item and sent to GŌKANA with the order.</p></div>}
