    ...
    <main className="pt-24 min-h-screen bg-bg text-ivory">
      ...
      <div className="container-gokana py-10 sm:py-14">
        ...
        <div className="bg-bg-alt border border-border rounded-2xl p-4 sm:p-5 mb-8 sm:mb-10 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {[{ id: 'all', label: 'All Gifts' }, ...occasions].map((occ) => {
              ...
              return (
                <button
                  ...
                  className={`flex-none px-3.5 py-1.5 sm:px-4 sm:py-2 text-[11px] sm:text-xs font-medium tracking-[0.06em] uppercase transition-all duration-200 whitespace-nowrap min-h-[38px] sm:min-h-[40px] flex items-center justify-center rounded-full ${
                    active
          ...
          {/* Sort & Count */}
          <div className="flex items-center justify-between md:justify-end gap-3 sm:gap-4 flex-shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-border">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="font-sans text-xs sm:text-sm text-ivory border border-border px-3 py-1.5 sm:px-3.5 sm:py-2 bg-bg focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent/30 min-h-[38px] sm:min-h-[40px] cursor-pointer rounded-lg"
              >
                {sortOptions.map((o) => (
        ...
        {loading ? (
          <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-7">
        ...
        <AnimatePresence mode="wait">
          {filtered.length === 0 ? (
            <motion.div
              ...
              className="text-center py-16 sm:py-20 bg-bg-alt border border-border rounded-2xl p-6 sm:p-10 max-w-lg mx-auto shadow-xl"
            >
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-accent/15 border border-accent/30 flex items-center justify-center text-accent mx-auto mb-4 sm:mb-5">
                <Sparkles size={24} />
              </div>
              <p className="font-serif text-xl sm:text-2xl font-light text-ivory mb-2">No gifts match this filter</p>
              <p className="font-sans text-xs sm:text-sm text-muted mb-6 font-light">
            ...
            <motion.div
              ...
              className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-7"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
